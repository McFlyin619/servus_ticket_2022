<template>
	<div>
		...
		<EditItem v-if="showEdit" :show="showEdit" :title="'Customer'" @close="showEdit = false" @saveEntry="saveEntry" :data="selectedItem"></EditItem>
		<div class="d-flex justify-content-between">
			...
			<div class="btn-group btn-group-sm align-self-center" role="group" aria-label="Small button group">
				...
				<button :disabled="!isItemSelected" @click="deleteItem" type="button" class="btn but-outline-delete" data-bs-toggle="tooltip" data-bs-placement="right" data-bs-custom-class="delete-custom-tooltip" data-bs-title="Delete selected customer">
					<i class="fas fa-trash-alt"></i>
				</button>
			</div>
		</div>
		<button @click="saveColumnOrder = true" type="button" class="btn but-outline-add float-end" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-custom-class="add-custom-tooltip" data-bs-title="Save current column order">
			Save Column Order <i class="fas fa-check-double"></i>
		</button>
		<grid-view :data="gridData" :columnDefs="columnDefs" :sizeColumns="sizeColumns" :darkMode="darkMode" :page="'customer'" :saveColumnOrder="saveColumnOrder" @itemSelected="itemSelected"></grid-view>
	</div>
</template>
